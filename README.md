...
8. **Бот запущен (polling), но на `/start` тишина**
   - Частая причина: у бота остался активный webhook от прошлого запуска/другого сервиса.
   - В текущей версии при старте вызывается `deleteWebhook`, но если вы запустили старый бинарник — пересоберите проект.
   - Проверьте, что процесс запущен именно из свежей сборки (`achievementbot.exe`) и что `BOT_TOKEN` относится к этому же боту.
